...
const DOMAINE_BORDER = {
  'Développement web':    'border-l-blue-400',
  'Design graphique':     'border-l-purple-400',
  'Rédaction':            'border-l-green-400',
  'Marketing digital':    'border-l-orange-400',
  'Traduction':           'border-l-teal-400',
  'Photographie':         'border-l-pink-400',
  'Assistance virtuelle': 'border-l-indigo-400',
  'Saisie de données':    'border-l-gray-400',
};
...
const MissionCard = ({ mission }) => {
  ...
  const domaineBorder = DOMAINE_BORDER[mission.domaine] || 'border-l-gray-300';
  ...
  const isRecent = mission.createdAt
    && (Date.now() - new Date(mission.createdAt).getTime()) < 3 * 24 * 60 * 60 * 1000;

  return (
    <Link to={`/missions/${mission.id}`} className="block group">
      <div className={`card h-full flex flex-col border-l-4 ${domaineBorder}`}>
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {isRecent && (
              <span className="badge bg-amber-100 text-amber-700 text-xs">Nouveau</span>
            )}
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="flex items-center gap-1.5 text-xs text-gray-500">
            <svg className="w-3.5 h-3.5 text-secondary-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
